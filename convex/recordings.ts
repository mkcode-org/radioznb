...
import { Expression, FilterExpression, SearchFilter } from 'convex/server'
...
export const list = query({
	...
	handler: async (ctx, args) => {
		...
		if (args.id) {
			query = query.filter((q) => q.eq(q.field('programId'), args.id))
		}

		const recordings = await query.order('desc').collect()
